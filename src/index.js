...
var TOOLS = {
  ...
  typeform_api__get_form_responses: {
    ...
    params: {
      ...
      after: { type: "string", description: "Response token for pagination.", required: false },
      before: { type: "string", description: "Response token for pagination.", required: false },
      completed: { type: "string", description: "Filter: true for completed, false for partial.", required: false },
      ...
      fields: { type: "string", description: "Comma-separated field IDs to include.", required: false },
    },
  },
  typeform_api__create_form: {
    ...
    params: {
      ...
      workspace_href: { type: "string", description: "Workspace API URL (e.g. https://api.typeform.com/workspaces/ABC123).", required: false },
      fields_json: { type: "string", description: "JSON string of fields array defining form questions.", required: false },
      settings_json: { type: "string", description: "JSON string of settings object.", required: false },
  ...
  typeform_api__update_form: {
    ...
    params: {
      ...
      fields_json: { type: "string", description: "JSON string of updated fields array.", required: false },
      settings_json: { type: "string", description: "JSON string of updated settings.", required: false },
  ...
  typeform_api__create_or_update_webhook: {
    ...
    params: {
      ...
      enabled: { type: "string", description: "Whether active: true or false.", required: false },
...
async function executeTool(env, name, args) {
  switch (name) {
    ...
    case "typeform_api__list_workspaces":
      return apiFetch(env, "GET", "/workspaces", { page: args.page, page_size: args.page_size, search: args.search });
    case "typeform_api__get_workspace":
      return apiFetch(env, "GET", "/workspaces/" + args.workspace_id);
    case "typeform_api__list_forms":
      return apiFetch(env, "GET", "/forms", { page: args.page, page_size: args.page_size, search: args.search, workspace_id: args.workspace_id });
    ...
    case "typeform_api__get_form_responses":
      return apiFetch(env, "GET", "/forms/" + args.form_id + "/responses", {
        ...
        after: args.after, before: args.before, completed: args.completed,
    ...
    case "typeform_api__create_form": {
      ...
      if (args.fields_json) body.fields = JSON.parse(args.fields_json);
      if (args.settings_json) body.settings = JSON.parse(args.settings_json);
      if (args.theme_href) body.theme = { href: args.theme_href };
      return apiFetch(env, "POST", "/forms", null, body);
    }
    case "typeform_api__update_form": {
      var body2 = {};
      if (args.title) body2.title = args.title;
      if (args.fields_json) body2.fields = JSON.parse(args.fields_json);
      if (args.settings_json) body2.settings = JSON.parse(args.settings_json);
      if (args.theme_href) body2.theme = { href: args.theme_href };
      return apiFetch(env, "PATCH", "/forms/" + args.form_id, null, body2);
    }
    ...
    case "typeform_api__list_themes":
      return apiFetch(env, "GET", "/themes", { page: args.page, page_size: args.page_size });
...
function buildSchema(toolName, toolDef) {
  var properties = {};
  var required = [];
  var keys = Object.keys(toolDef.params);
  for (var i = 0; i < keys.length; i++) {
    var k = keys[i];
    var v = toolDef.params[k];
  ...
  if (required.length > 0) schema.inputSchema.required = required;
  return schema;
}
...
async function handleRpc(request, env) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ jsonrpc: "2.0", id: null, error: { code: -32700, message: "Parse error" } }), { headers: { "Content-Type": "application/json" } });
  }
  ...
  if (method === "notifications/initialized") {
    return new Response(JSON.stringify({ jsonrpc: "2.0", id: id, result: {} }), { headers: { "Content-Type": "application/json" } });
  }
  ...
  if (method === "tools/list") {
    ...
    for (var i = 0; i < toolNames.length; i++) {
      tools.push(buildSchema(toolNames[i], TOOLS[toolNames[i]]));
    }
    return new Response(JSON.stringify({
      jsonrpc: "2.0",
      id: id,
  ...
  if (method === "tools/call") {
    ...
    if (!TOOLS[toolName]) {
      return new Response(JSON.stringify({ jsonrpc: "2.0", id: id, error: { code: -32601, message: "Unknown tool: " + toolName } }), { headers: { "Content-Type": "application/json" } });
    }
    var result = await executeTool(env, toolName, toolArgs);
    return new Response(JSON.stringify({
      jsonrpc: "2.0",
      id: id,
  ...
  return new Response(JSON.stringify({ jsonrpc: "2.0", id: id, error: { code: -32601, message: "Method not found: " + method } }), { headers: { "Content-Type": "application/json" } });
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health" && request.method === "GET") {
      return new Response(JSON.stringify({ status: "ok", tools: Object.keys(TOOLS).length }), { headers: { "Content-Type": "application/json" } });
    }
